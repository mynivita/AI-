...
st.set_page_config(page_title="AI 차트 백테스터 V2", layout="wide")

# ----------------------------- 다크 테마 -----------------------------
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans+KR:wght@400;500;700&family=IBM+Plex+Mono:wght@500;600&display=swap');
html, body, .stApp { background-color:#0a0d12 !important; color:#cfd5dd !important;
  font-family:'IBM Plex Sans KR', sans-serif !important; }
[data-testid="stSidebar"] { background-color:#0e1319 !important;
  border-right:1px solid #1d2530; }
[data-testid="stSidebar"] * { color:#c3cad4; }
h1, h2, h3 { font-family:'IBM Plex Mono','IBM Plex Sans KR',monospace !important;
  color:#d9a441 !important; letter-spacing:-0.5px; }
[data-testid="stMetricValue"] { font-family:'IBM Plex Mono',monospace !important;
  color:#e8ecf1 !important; }
[data-testid="stMetricLabel"] { color:#8b95a3 !important; }
.stButton>button { background:#d9a441 !important; color:#0a0d12 !important;
  font-weight:700 !important; border:none !important; }
[data-testid="stExpander"] { background:#0e1319;
  border:1px solid #1d2530 !important; border-radius:4px; }
hr { border-color:#1d2530 !important; }
[data-testid="stCaptionContainer"], small { color:#77808d !important; }
[data-testid="stAlert"] { background:#12161d !important;
  border:1px solid #1d2530 !important; color:#c3cad4 !important; }
</style>
...
def _dark(fig):
    ...
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)",
                      ...
                      font=dict(color="#c3cad4", family="IBM Plex Sans KR"))
    fig.update_xaxes(gridcolor="#1b222d", zerolinecolor="#1b222d",
                     linecolor="#1b222d")
    fig.update_yaxes(gridcolor="#1b222d", zerolinecolor="#1b222d",
                     linecolor="#1b222d")
    return fig
...
def search(query_close, query_value, window, vol_weight,
    ...
    dot = Mp @ qp
    corr_p = dot / window
    # (Mp - qp)² 임시 배열(수백 MB)을 만들지 않는 계산: |a-b|² = |a|²+|b|²-2ab
    row_sq = np.einsum("ij,ij->i", Mp, Mp)
    dist_p = np.sqrt(np.maximum(
        row_sq + float(qp @ qp) - 2.0 * dot, 0.0) / window)
...
def candle_fig(df, title, highlight=None, height=400):
    ...
    fig.add_trace(go.Candlestick(
        ...
        increasing_line_color="#e5484d", decreasing_line_color="#4f83f1"),
        row=1, col=1)
    fig.add_trace(go.Bar(x=df["date"], y=df["value"],
                         marker_color="#3a4454"), row=2, col=1)
    if highlight:
        fig.add_vrect(x0=highlight[0], x1=highlight[1], fillcolor="#d9a441",
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**시나리오 백테스트**")
    tp_pct = st.number_input("목표가 TP (+%)", min_value=1.0, max_value=100.0,
                             value=15.0, step=1.0)
    sl_pct = st.number_input("손절가 SL (-%)", min_value=1.0, max_value=50.0,
                             value=7.0, step=0.5)
    st.caption(f"보유 기간은 '패턴 이후 분석 기간'({after_days}거래일)을 따릅니다.")
    run = st.button("유사 차트 검색", width='stretch', type="primary")
...
st.subheader(f"패턴 통계 — 유사 상위 {n_stats}개 구간의 이후 {after_days}거래일")
if len(pat_ret) < 30:
...
sfig = _dark(sfig)
event = st.plotly_chart(sfig, width='stretch',
    ...
    for k, i in enumerate(b["members"][:show_n]):
        ...
        with bcols[k % 2]:
            st.plotly_chart(candle_fig(
                ...
                width='stretch', key=f"binchart_{sel}_{k}")
...
st.caption("과거 통계일 뿐 미래를 보장하지 않습니다. 생존편향이 내장되어 있어 "
...
if len(rob) >= 3:
    ...
    if sign_flip:
        s_verdict = (":red[**불안정**] — 표본 수에 따라 전체 평균보다 좋았다 나빴다가 "
                     "뒤집힙니다. 이 패턴의 통계는 우연의 영향이 커서 신뢰하기 어렵습니다.")
    elif spread < 0.03:
        s_verdict = (":green[**안정적**] — 표본을 50개로 줄여도 300개로 늘려도 결과가 "
                     "거의 같습니다. 이 패턴군의 통계는 소수 극단값에 좌우되지 않습니다.")
    elif spread < 0.07:
        s_verdict = (":orange[**보통**] — 표본 수에 따라 수치가 다소 움직이지만 방향은 "
                     "일관됩니다. 참고는 하되 수치 자체를 믿지는 마세요.")
    else:
        s_verdict = (":red[**불안정**] — 표본 수에 따라 평균이 크게 출렁입니다. "
...
# 4) 시나리오 백테스트: 유사 구간에서 TP/SL 선착 판정
st.subheader(f"시나리오 백테스트 — TP +{tp_pct:.1f}% / SL -{sl_pct:.1f}% / "
             f"보유 {after_days}거래일")
tp_r, sl_r = tp_pct / 100.0, sl_pct / 100.0
n_tp = n_sl = n_open = 0
outcomes, open_rets = [], []
for i in picked[:n_stats]:
    mcode, s = meta[i]
    g = stocks[mcode]
    e = s + window
    if e >= len(g):
        continue
    entry = float(g["close"].iloc[e - 1])
    hi = g["high"].to_numpy(float)
    lo = g["low"].to_numpy(float)
    cl = g["close"].to_numpy(float)
    end = min(len(g) - 1, e - 1 + after_days)
    res = None
    for d in range(e, end + 1):
        hit_tp = np.isfinite(hi[d]) and hi[d] >= entry * (1 + tp_r)
        hit_sl = np.isfinite(lo[d]) and lo[d] <= entry * (1 - sl_r)
        if hit_sl:            # 같은 날 동시 터치 포함 → 보수적으로 손절 처리
            res = -sl_r
            n_sl += 1
            break
        if hit_tp:
    ...
    if res is None:
        res = cl[end] / entry - 1
        n_open += 1
        open_rets.append(res)
    outcomes.append(res)

tot = len(outcomes)
if tot < 30:
    st.warning(f"판정 가능한 표본이 {tot}개뿐이라 시나리오 통계 신뢰도가 낮습니다.")
if tot > 0:
    ev = float(np.mean(outcomes))
    breakeven = sl_r / (tp_r + sl_r)  # 미결 제외 시 손익분기 TP 선착률
    d1, d2, d3, d4, d5 = st.columns(5)
    d1.metric("판정 표본", f"{tot}개")
    d2.metric("TP 선착", f"{n_tp}건 ({n_tp/tot*100:.0f}%)")
    d3.metric("SL 선착", f"{n_sl}건 ({n_sl/tot*100:.0f}%)")
    om = f" (평균 {np.mean(open_rets)*100:+.1f}%)" if open_rets else ""
    d4.metric("기간 내 미결", f"{n_open}건 ({n_open/tot*100:.0f}%)" )
    d5.metric("거래당 기대값 (EV)", f"{ev*100:+.2f}%")
    if open_rets:
        st.caption(f"미결 {n_open}건은 보유 종료 시점 수익률로 정산 (평균 {np.mean(open_rets)*100:+.1f}%)")

    if ev > 0.01:
        sc_verdict = (f":green[**과거 기준 유리**] — 닮은 구간들에서 이 손익비 설정은 "
                      f"거래당 평균 {ev*100:+.2f}%를 남겼습니다.")
    elif ev > -0.01:
        sc_verdict = (":orange[**과거 기준 본전 수준**] — 이 설정의 역사적 기대값은 "
                      "0 근처입니다. 수수료·슬리피지를 감안하면 우위가 없습니다.")
    else:
        sc_verdict = (f":red[**과거 기준 불리**] — 닮은 구간들에서 이 손익비 설정은 "
                      f"거래당 평균 {ev*100:+.2f}%를 잃었습니다.")
    decided = n_tp + n_sl
    if decided > 0:
        sc_verdict += (f" 참고: 이 TP/SL 비율의 손익분기 TP 선착률은 "
                       f"{breakeven*100:.0f}%이며, 실제(미결 제외)는 "
                       f"{n_tp/decided*100:.0f}%였습니다.")
    st.markdown(f"> {sc_verdict}")
    st.caption("같은 날 TP·SL 동시 도달은 장중 순서를 알 수 없어 손절로 간주(보수적). "
               "수수료·세금·슬리피지 미반영. TP/SL을 이리저리 바꿔 좋은 기대값을 "
               "찾아내는 것은 과최적화이므로 시나리오를 먼저 정한 뒤 검증만 하세요.")
...
for rank, i in enumerate(picked[:top_k], 1):
    ...
    with col1:
        st.plotly_chart(candle_fig(
            ...
            highlight=hl, height=380), width='stretch')
    with col2:
